...
export default function DeleteScheduleButton({
  ...
  async function onClick() {
    try {
      setIsFetching(true);
      const response = await fetch(`/api/admin/teachers/${schedule.id}`, {
        method: "DELETE",
      });
      ...
      toast({
        title: "Success",
        description: `Deleted teacher: ${schedule.date}`,
      });
    } catch (error) {
      console.error(error);
      toast({
  ...
  return (
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Delete: {schedule.id}?</AlertDialogTitle>
          <AlertDialogDescription>
